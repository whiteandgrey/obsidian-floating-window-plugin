...
class FloatingWindowPlugin extends obsidian_1.Plugin {
    constructor() {
        super(...arguments);
        ...
        if (activeLeaf && activeLeaf.view && 'editor' in activeLeaf.view) {
            ...
            if (editor) {
                ...
                if (selectedText.trim()) {
                    ...
                    new obsidian_1.Notice('Please select some text first');
                }
            }
        }
    }
    async openFloatingWindow(text, selectionRect, sourceLeaf) {
    ...
    cleanupOrphanedWindows() {
        ...
        for (const window of this.floatingWindows) {
            if (window.sourceLeaf) {
                ...
                const leafStillOpen = openLeaves.some((leaf) => {
                    // Check by reference
                    if (leaf === sourceLeaf)
                        return true;
                    // Check by view existence
                    if (leaf.view && sourceLeaf.view && leaf.view === sourceLeaf.view)
                        return true;
                    return false;
                });
    ...
    ensureWindowWithinBounds(windowElement) {
    ...
    setWindowPosition(windowElement, selectionRect) {
        if (selectionRect) {
    ...
    jumpToOriginalParagraph(text, selectionRect) {
        try {
            const activeLeaf = this.app.workspace.activeLeaf;
            if (activeLeaf && activeLeaf.view && 'editor' in activeLeaf.view) {
                const editor = activeLeaf.view.editor;
...
exports.default = FloatingWindowPlugin;
;
class FloatingWindowSettingTab extends obsidian_1.PluginSettingTab {
